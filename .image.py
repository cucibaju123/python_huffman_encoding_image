...
an_image = Image.open(input)
width, height = an_image.size
image_sequence = an_image.getdata()
image_array = np.array(
    image_sequence
)  # is of the form [[r1,g1,b1],[r2,g2,b2],[r3,g3,b3],...] where ri,bi and gi are intensity values of respective rgb colors
...
ListOfAllNodes = (
    []
)  # All Nodes with unique intensity values will be stored in this list
for pixVal in IntensityCounter.keys():
    ListOfAllNodes.append(Node(pixVal, IntensityCounter[pixVal]))

ListOfAllNodes2 = copy.deepcopy(
    ListOfAllNodes
)  # The above list will be used to build the tree. So,let's have a copy in which we update the huffman codes

heapq.heapify(
    ListOfAllNodes
)  # Heapifying the list gives the Node with the smallest frequency
...
def CodeTheImage(imageArray, ListOfAllNodes2, width, height):
    ...
    for t in ListOfAllNodes2:
        encodedImageString += extraPaddingOf0sInFront(
            8, bin(t.intensityVal)[2:]
        )  # 8 bits
        assert len(bin(t.intensityVal)[2:]) <= 8
        # 3 bits
        encodedImageString += extraPaddingOf0sInFront(8, bin(len(t.code) - 1)[2:])
